...
static void handle_update_mech_setting(ssm2_command_t const * cmd)
{
     ...
     */
    if (cmd->len != sizeof(us1_jp1_conf_t) + HISTORY_PAYLOAD_LEN_MAX)
    {
        result = CMD_RESULT_INVALID_FORMAT;
    }
    else
    {
        ...
        memcpy(&history.lock_target_before, ble_ssm2_get_mech_setting(), sizeof(us1_jp1_conf_t));
        ...
        case NRF_SUCCESS:
        {
            ...
            memcpy(&history.lock_target_after, cmd->data, sizeof(us1_jp1_conf_t));
            memcpy(history.payload, cmd->data + cmd->len - HISTORY_PAYLOAD_LEN_MAX, HISTORY_PAYLOAD_LEN_MAX);
            STATIC_ASSERT(offsetof(history_mech_setting_updated_t, lock_target_after) == offsetof(history_mech_setting_updated_t, lock_target_before) + sizeof(us1_jp1_conf_t));
